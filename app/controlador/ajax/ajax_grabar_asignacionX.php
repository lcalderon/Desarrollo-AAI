...
/* DATOS QUE VIENEN COMO PARAMETROS   */
...
$prioridad = $_POST[PRIORIDAD];  // SI ES EME o PRO
...
$con->update("$con->temporal.asistencia_asig_proveedor",$rowsu," WHERE IDASISTENCIA='$idasistencia' AND STATUSPROVEEDOR='AC'");
...
/*  DATOS A GRABARSE EN LA ASIGNACION DE PROVEEDOR */
$asig[IDPROVEEDOR]=$idproveedor;
$asig[IDASISTENCIA]=$idasistencia;
$asig[STATUSPROVEEDOR]='AC';  // POR DEFAULT AL ASIGNAR ASUME QUE ES ACTIVO AC
...
else      // EN CASO DE $prioridad == 'PRO'
{
	...
	if($nreg_prov==0)
	{
		...
		for($i=1;$i<=$monprog;$i++){
			...
			if($rset_monitoreo=$exec_monitoreo->fetch_object())
			{
				if($i<$monprog)	$rowtareaprov[IDTAREA]='MON_PROV';
				else{
					$rowtareaprov[IDTAREA]='ARR_PROV';
				}
				$rowtareaprov[FECHATAREA]=$rset_monitoreo->MONITOREO;
