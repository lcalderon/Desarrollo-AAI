<?php
	...
	$sql_hogar="SELECT
				...
				WHERE catalogo_proveedor.ACTIVO = 1
					AND monitoreo_servicio_programado.TIPO = 'PC'
...
<body>
	<table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
		<tr>
			...
			<td width="86%"><div align="center"><h1 style="color:#000066"><?=_('MONITOR SERVICIOS PROGRAMADOS - PC');?></h1></div></td>
			<td width="2%"><? if(validar_permisos("ASIGNAR_PROV_SERVPROG")){ ?><img src="../../../../imagenes/iconos/adds.gif" style="cursor:pointer" onclick="ventana_cambios()" width="9" height="11" title="<?=_('PRESENTACION');?>" alt="<?=_('PRESENTACION');?>"><? } ?></td>
	  </tr>
	</table>

	<table border="1" width="100%"   style="border:1px solid #000066;border-collapse:collapse" cellspacing="1" cellpadding="1">
		<tr bgcolor="#000066">
			<td colspan="4" bgcolor="#000066"><div align="center"><h2 style="color:#FFFFFF"><?=_('TECNICOS PC ASISTENCIA');?></h2></div></td>
		</tr>
		<tr bgcolor="#000066">
		...
		if($numreg >0)
		 {	
			...
			while($reg = $result->fetch_object())
			 {
				$ii++;
				 if($ii%2==0) $fondo='#FFFFFF'; else $fondo='#F2FFF2';
	?>	
		<tr bgcolor="<?=$fondo;?>">
				...
				$sql_hogar_info="SELECT
						...
						$numinfo=$rs_info->num_rows*1;
				 		while($row = $rs_info->fetch_object()){		
							$info[$i]=$info[$i]."<font color='#DD0000'>".$row->tiempo1."-".$row->tiempo2." / ".$row->distrito."</font><br>"."<font color='#002F5E'>".$row->idusuarioresponsable." - ".$row->idasistencia."</font><br>";	
						}
					}
				?>
	...
	<script type="text/javascript">	
		...
		function ventana_cambios(){
			...
			else
			{
				win = new Window({
					className: "alphacube",
					title: '<?=_("MODIFICAR PRESENTACION - PC")?>',
					width: 640,
					height: 420,
