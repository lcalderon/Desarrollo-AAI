<?php
	...
	$sql_hogar="SELECT
				...
				WHERE catalogo_proveedor.ACTIVO = 1
					AND monitoreo_servicio_programado.TIPO = 'INSPECTOR'
...
<html>
	<head>
		<title><?=_('Monitoreo Inspectores');?></title>
...
<body>
	<table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
		<tr>
			...
			<td width="86%"><div align="center"><h1 style="color:#000066"><?=_('MONITOR SERVICIOS PROGRAMADOS - INSPECTORES');?></h1></div></td>
	...
	<table border="1" width="100%"   style="border:1px solid #000066;border-collapse:collapse" cellspacing="1" cellpadding="1">
		<tr bgcolor="#000066">
			<td colspan="4" bgcolor="#000066"><div align="center"><h2 style="color:#FFFFFF"><?=_('INSPECTORES ASISTENCIA');?></h2></div></td>
		</tr>
		<tr bgcolor="#000066">
			<td><div align="center"><span class="style4"><?=_('INSPECTORES');?></span></div></td>
		...
		if($numreg >0)
		 {	
			...
			while($reg = $result->fetch_object())
			 {
				$ii++;
				 if($ii%2==0) $fondo='#FFFFFF'; else $fondo='#FFF2F2';
	?>	
		<tr bgcolor="<?=$fondo;?>">
	...
	<?
			 for ( $i = 1 ; $i <= 3 ; $i ++){
				...
				$sql_grua_info="SELECT
                              /* servicios_programados_grua */
							  cp2.nombrefiscal,
							  asis.idasistencia,
							  asigp.teat,
							  asigp.team,
							 ...
							 /* $var_entidad AS ENTIDADES,*/
							  ...
							  IF( asisl.cveentidad3 != 0, ent3.descripcion, '')) AS direcTotal,
							...
							WHERE asigp.idasistencia = asis.idasistencia
								...
								AND asigp.idproveedor IN(SELECT DISTINCT
														 ...
														 WHERE cp.idproveedor = cps.idproveedor
															 ...
															 AND cs.idfamilia = 1 /* vehicular */														
															 AND cp.activo = 1
															 AND cp.nombrefiscal NOT LIKE '%REEMBOLSO%'
															 AND cp.idproveedor =$reg->IDPROVEEDOR
														 ORDER BY 1)";
						
						$rs_info=$con->query($sql_grua_info);
						$numinfo=$rs_info->num_rows*1;
						$var=0;
				 		while($row = $rs_info->fetch_object()){
							$var++;
								$rsubi=$con->query("SELECT * FROM  $con->temporal.asistencia_vehicular_remolque_destino WHERE IDASISTENCIA=$row->idasistencia ORDER BY ID DESC LIMIT 1");
								if($rowubi = $rsubi->fetch_object()){
								 
										$unidad = new unidadfederativa();
										$entidad[1]=$rowubi->CVEENTIDAD1;
										$entidad[2]=$rowubi->CVEENTIDAD2;
										$entidad[3]=$rowubi->CVEENTIDAD3;
										$entidad[4]=$rowubi->CVEENTIDAD4;
										$entidad[5]=$rowubi->CVEENTIDAD5;
										$entidad[6]=$rowubi->CVEENTIDAD6;
										$entidad[7]=$rowubi->CVEENTIDAD7;
										$arr = $unidad->nombre_entidades_array($entidad);
										
										$destino=$arr[1]."/".$arr[2]."/".$arr[3];										 
										$destino=(count($arr[3]) =="")?"S/D":$destino;
								}
						  
							if($var < $numinfo ) $separador="---------------------------------<br>";
							
							$info[$i]=$info[$i]."Origen :<font color='#DD0000' size='1px'>".$row->direcTotal."</font><br>Destino:<font color='#DD0000' size='1px'>$destino</font><br>"."<font color='#002F5E'>".$row->idusuarioresponsable." - <a href='../../plantillas/etapa1_1.php?idasistencia=$row->idasistencia' target='_blank'>".$row->idasistencia."</a></font><br>$separador";
							$destino="";
							$separador="";
	...
	<script type="text/javascript">	
		...
		function ventana_cambios(){
			...
			else
			{
				win = new Window({
					className: "alphacube",
					title: '<?=_("MODIFICAR PRESENTACION - INSPECTORES")?>',
					width: 640,
					height: 420,
					...
					resizable: true,
					opacity: 0.95,
					url: 'paginar_vehicular_inspectores/'
				});
