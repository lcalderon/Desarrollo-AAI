<?php
	...
	$result=$con->query("SELECT * FROM $con->temporal.delegar_asistencia_log WHERE IDASISTENCIA='".$_GET["asistencia"]."' AND IDASISTENCIA!=0 ORDER BY FECHAMOD DESC");
...
<body>
        ...
        <table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable" style="width:100%">
            <thead>
                <tr>
                    <th><h3><?=_("ID") ;?></h3></th>
                    <th><h3><?=_("IDASISTENCIA") ;?></h3></th>
                    <th><h3><?=_("IDUSUARIOMOD") ;?></h3></th>
                    <th><h3><?=_("FECHAMOD") ;?></h3></th>
                    <th><h3><?=_("DELEGADOR") ;?></h3></th>
                    <th><h3><?=_("DELEGADOHACIA") ;?></h3></th>
                    <th class="nosort"><h3><?=_("ACCION") ;?></h3></th>
                </tr>				
            </thead>
            <tbody>
		...
		?>		
		 <tr>
			...
			<td align="center"><?=utf8_encode($reg->IDASISTENCIA);?></td>			
			 
			<td align="center"><?=$reg->IDUSUARIOMOD;?></td>
			<td align="center"><?=$reg->FECHAMOD;?></td>
			<td align="center"><?=$reg->USUARIODELEGADOR;?></td>
			<td align="center"><?=$reg->USUARIODELEGADOHACIA;?></td>
			<td><?=$arr_motivodelegar[$reg->ARRMOTIVO];?></td>
