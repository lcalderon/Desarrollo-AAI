...
$modo='';
$statusautorizaciondesvio='2';
$annio=(isset($_POST[ANNIO]))?$_POST[ANNIO]:$fecha_act[year];
$mes = (isset($_POST[MES]))?$_POST[MES]:$fecha_act[mon];
...
validar_permisos("MENU_CONSULTAHIST",1);

//sorter.search('estado');
?>
<title><?=_('EXP. HISTORICOS')?></title>
	<script type="text/javascript" src="../../../../estilos/functionjs/func_global.js"></script>
<div  style="width:100%">
<body onload="marcar_etapasseleccionadas();marcar_cuentasseleccionadas();cambio_option();">   
<form  id='datos_busqueda' method ='post'>
 <span><font size="4px"><?=_("ASISTENCIAS BRINDADAS EN EXPEDIENTES CERRADOS")?></font>
	...
	<table style="width:100%">
    	<tr>
      		<td ><?=_('BUSCAR EN COLUMNAS')?></td>
       		...
       		<td ><?=_('ASISTENCIA')?></td>
       		<td ><?=_('FECHA')?></td>
       		<td  colspan="2"><?//=_('NUMERO DE ASISTENCIA')?></td>
    	...
    	<tr>
       		...
       		<td style="display:none"><? $asis->cmbselect_ar('ARRSTATUSEXPEDIENTE',$desc_status_expeduiente,"$arrstatusexpediente",'id="arrstatusexpediente"',"onchange=cambio_option()","" ); ?></td>
       		<td valign="top">
        ...
        <table style="width:100%" >
           	<tr>
            		...
            		<?=_('ETAPAS')?>
 	         		...
 	         		?>
 	         		->&nbsp;&nbsp;
  	         		<?=_('TODOS')?><input type="checkbox"  name='TODASETAPAS' id='todasetapas' onclick="marcar_todasetapas()"  <?=$checked?>/>
            		</td>
               	...
               	<tr>
            		...
            		<td  valign="top"  align="right">
            			<img src='/imagenes/64x64/Search.png' onclick="buscar()" title="<?=_('BUSCAR')?>" align='absbottom' border='1' style='cursor: pointer;' />
        			</td>
...
<table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable" width="100%">
  ...
  <tbody >
    ...
    while ($reg=$lista_asis->fetch_object()){
    	...
    	echo "<td>
					<img src='/imagenes/iconos/editars.gif' title='Editar' align='absbottom' border='0' style='cursor: pointer;' onclick=abrir_pestana('$reg->IDEXPEDIENTE','$varexis') >
					...
					<img src='/imagenes/iconos/delegar.png' alt='16px' width='16px' title='Delegaciones' align='absbottom' border='0' style='cursor: pointer;' onClick=\"presentar_formulario('','../delegar/historial_delegar.php','CIERRE LA VENTANA ANTERIOR','HISTORIAL DE DELEGACION','740','300','','','$reg->IDASISTENCIA')\" >
		</td>
    	";
    	echo "</tr>	";
    }
...
{
	var url= '../../expediente/entrada/expediente_frmexpediente.php?idexpediente='+idexpediente+'&origen=HISTORICOEXP&varexis='+varexis;
