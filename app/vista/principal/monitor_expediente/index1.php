...
$user= (isset($_GET[IDUSUARIO]))?$_GET[IDUSUARIO]:$_SESSION[user];
$arrstatusexpediente = 'PRO';// (isset($_GET[ARRSTATUSEXPEDIENTE])?$_GET[ARRSTATUSEXPEDIENTE]:'PRO');
$arrstatusasistencia = (isset($_GET[ARRSTATUSASISTENCIA])?$_GET[ARRSTATUSASISTENCIA]:'');
$modo='';
$statusautorizaciondesvio=0;
/* FECHA 1 */
$annio=(isset($_GET[ANNIO]))?$_GET[ANNIO]:$fecha_act[year];
$mes = (isset($_GET[MES]))?$_GET[MES]:'';
$dia = (isset($_GET[DIA]))?$_GET[DIA]:'';
$hora = (isset($_GET[HORA]))?$_GET[HORA]:'';

/* FECHA 2 */
$annio2=(isset($_GET[ANNIO2]))?$_GET[ANNIO2]:'';
$mes2 = (isset($_GET[MES2]))?$_GET[MES2]:'';
$dia2 = (isset($_GET[DIA2]))?$_GET[DIA2]:'';
$hora2 = (isset($_GET[HORA2]))?$_GET[HORA2]:'';


if ($_GET[IDETAPA]){
	foreach ($_GET[IDETAPA] as $etapa) if ($etapa!='') $etapas[] ="'$etapa'";
...
if ($_GET[IDCUENTA])
{
	foreach ($_GET[IDCUENTA] as $cuenta) if ($cuenta!='')$cuentas[] ="'$cuenta'";
...
if ($_GET[IDUSUARIO])
{
	foreach ($_GET[IDUSUARIO] as $usuario)
	if ($usuario!=''){
		$usuarios[]="'$usuario'";
...
if ($_GET[COLUMNAGEN]!='' && $_GET[TEXTCOLUMNAGEN]!='')
{
	$campo=$_GET[COLUMNAGEN];
	$dato= $_GET[TEXTCOLUMNAGEN];
}
else
{
	$campo=$_GET[COLUMNA];
	$dato= $_GET[TEXTCOLUMNA];
}


/*  lista de asistencias  a presentar en el grid */
$lista_asis = $asis->listar_asistencias($idusuarios,$arrstatusexpediente,$arrstatusasistencia,$idetapas,$idcuentas,$modo,$statusautorizaciondesvio,$annio,$mes,$dia,$hora,$annio2,$mes2,$dia2,$hora2,$idasistencia,'',$campo,$dato);
 ...
 <span><font size="4px"><?=_("ASISTENCIAS BRINDADAS Y EN PROCESO")?></font>
	 <span style="float:right;">
 		<select name="COLUMNAGEN" id='columnagen'>
 			<option value='a.IDASISTENCIA' <?=($_GET[COLUMNAGEN]=='a.IDASISTENCIA')?'selected':'';?>><?=_('ASISTENCIA')?></option>
			<option value='a.IDEXPEDIENTE' <?=($_GET[COLUMNAGEN]=='a.IDEXPEDIENTE')?'selected':'';?>><?=_('EXPEDIENTE')?></option>
 		</select>
 ...
 <fieldset>
    	   		...
    	   		<tr>
	    	   		<td valign="top" >
						<select name='COLUMNA' id="columna">
							<option value='NOM_PROVEEDOR' <?=($_GET[COLUMNA]=='NOM_PROVEEDOR')?'selected':'';?>><?=_('PROVEEDOR')?></option>
							<option value='NOM_AFILIADO' <?=($_GET[COLUMNA]=='NOM_AFILIADO')?'selected':'';?>><?=_('AFILIADO')?></option>
							<option value='NOM_SERVICIO' <?=($_GET[COLUMNA]=='NOM_SERVICIO')?'selected':'';?>><?=_('SERVICIO')?></option>
						</select>
    	    			<input type="text"  name="TEXTCOLUMNA" id="textcolumna" size='40'  value="<?=$_GET[TEXTCOLUMNA]?>"/>
    	       		</td>
         ...
         <table style="width:100%;" >
            	<tr>
            		...
            		<?  $checked='checked';
            		if ($_GET[IDCUENTA])
            		if ($_GET[TODASCUENTAS]!='on') $checked='';
 	         		?>
 	         		->&nbsp;&nbsp;
            		...
            		<?=_('ETAPAS')?>
  	           		<?  $checked='checked';
  	           		if ($_GET[IDETAPA])	if ($_GET[TODASETAPAS]!='on') $checked='';
 	         		?>
 	         		->&nbsp;&nbsp;
            		...
            		<?  $checked='';
            		if ($_GET[IDUSUARIO])
            		if ($_GET[TODOUSUARIOS]=='on') $checked='checked';
            		?>
            		->&nbsp;&nbsp;
               	...
               	<tr>
               	    ...
               	    <?foreach($cuentas_permitidas as $indice=>$cta):?>
               	   	   <? if (in_array($indice,$_GET[IDCUENTA])) $selected='selected';	else $selected='';?>
               		...
               		<?foreach($etapass as $indice=>$etapa):?>
               		  <? if (in_array($indice,$_GET[IDETAPA])) $selected='selected'; else $selected='';?>
