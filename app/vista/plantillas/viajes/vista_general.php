...
<script type="text/javascript">
	...
	function validar_datos_generales(){
		...
		if ($F('lugardelevento')==''){
			...
			$('lugardelevento').focus();
		} else if ($F('arrcondicionservicio')=='GAR' && $F('garantia_rel')==''){
			alert("<?=_('INGRESE NUMERO DE ASISTENCIA RELACIONADA A LA GARANTIA')?>");
		} else if ($F('fechaorigen') ==''){
			alert("<?=_('INGRESE LA FECHA DE SALIDA')?>");	
		} else if ($F('fechadestino') ==''){
			alert("<?=_('INGRESE LA FECHA DE RETORNO')?>");			
		} else if ($F('fechaorigen') > $F('fechadestino')){
			alert("<?=_('RANGOS DE FECHAS NO VALIDOS')?>");
	...
	function setupCalendarsesc1(){
	  
			Calendar.setup(
			  {
				dateField: 'fechaorigen',
				triggerElement: 'calendarButtonesc1'
			
			  }
		  );		  
	}
	
	function setupCalendarsesc2(){
	  
			Calendar.setup(
			  {
				dateField: 'fechadestino',
				triggerElement: 'calendarButtonesc2'
