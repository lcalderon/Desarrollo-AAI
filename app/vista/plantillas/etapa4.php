...
<div id='barra' class="barraestado" style="float:right">
	...
	<table align="right">
		<tbody>
			...
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td><input type='button' id='siguiente' value="<?=_('MONITOREO PROV').' >>'?>" class='guardar' onClick="siguiente_etapa()" <?=$disabled?> ></td>
			</tr>
		</tbody>
	</table>
		<? }?>	
	...
	if (trim($F('comentario'))!='')
	{
		new Ajax.Request('/app/controlador/ajax/ajax_grabar_bitacora_monitoreo_proveedor.php?proveedor_act='+prov,
		{
			...
			onComplete: function(t)
			{
				...
				new Ajax.Request('/app/controlador/ajax/ajax_grabar_deficienciasE4.php',
				{
					method:'post',
					parameters:{
