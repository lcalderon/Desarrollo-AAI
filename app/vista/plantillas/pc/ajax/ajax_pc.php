...
/* DATOS PARA LA TABLA PC */
$pc[IDASISTENCIA]= $idasistencia;
$pc[IDCONTACTANTE]= $_POST[IDCONTACTANTE];
$pc[IDSERVICIO]=$_POST[IDSERVICIO];
$pc[TIPOINMUEBLE]=$_POST[TIPOINMUEBLE];
$pc[ARRPRIORIDADATENCION]=$_POST[ARRPRIORIDADATENCION];
$pc[MARCA]=$_POST[MARCA];
$pc[MODELO]=$_POST[MODELO];
$pc[NUMEROSERIE]=$_POST[NUMEROSERIE];
$pc[SISTEMAOPERATIVO]=$_POST[SISTEMAOPERATIVO];
$pc[FECHADECOMPRA]=$_POST[ANIO].'-'.$_POST[MES].'-'.$_POST[DIA];
$pc[IDUSUARIOMOD]=$_POST[IDUSUARIO];

if ($_POST[IDASISTENCIA]=='')
{
	$con->insert_reg($con->temporal.'.asistencia_pc',$pc);
}
else {
	$con->update($con->temporal.'.asistencia_pc',$pc," where IDASISTENCIA = '$_POST[IDASISTENCIA]'  ");
