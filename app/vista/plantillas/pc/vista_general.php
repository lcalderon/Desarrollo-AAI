 ...
 <form name='form_datos_generales' id='form_datos_generales'  >
	...
	<table>
		...
		<tr>
			...
			<td><? $con->cmbselect_ar('TIPOINMUEBLE',$desc_tipo_inmueble,$asis->asistencia_familia->TIPOINMUEBLE,'id=lugar','','') ?></td>
		</tr>	
	</table>
		<div id='div-pcs'><? include("datospcs.php"); ?></div>
...
function validar_datos_generales(){
	...
	else if ($F('marca')=='') alert("<?=_('INGRESE LA MARCA')?>");
	else if ($F('modelo')=='') alert("<?=_('INGRESE EL MODELO')?>");
	else sw = true;
	return sw ;
}
...
function copiar_direccion(){
	...
	function cargar_pcs(idvalor)
	{	
		new Ajax.Updater('div-pcs', 'pc/datospcs.php', {	
			parameters: { idpc: idvalor },
			method: 'post'

			});
	} 	
	 
	function ventana_pcs(){
		
		var cve_id=document.form_datos_generales.cveafiliado.value;
		window.open("pc/frm_pcs.php?cve_id="+cve_id,"mediop","height=300, width=800,left=100,top=0,resizable=no,scrollbars=yes,toolbar=no,status=yes");
