...
<input type='hidden' name='IDEXPEDIENTE' id='idexpediente' value="<?=$exp->idexpediente ?>"> 
	...
	<table>
		...
		<tr>
			...
			<td><? $con->cmbselect_ar('ARRTIPOINMUEBLE',$desc_tipo_inmueble,$asis->asistencia_familia->TIPOINMUEBLE,'id=lugar','','') ?></td>
		</tr>
		<tr>
			<td><?=_('NOMBRE DE LA MASCOTA').' *'?></td>
			<td><input type='text' name='NOMBREMASCOTA' id='nombremascota' value="<?=$asis->asistencia_familia->NOMBREMASCOTA?>" size="40"></td>
		</tr>
		<tr>
			<td><?=_('EDAD MASCOTA').' *'?></td>
			<td><input type='text' name='EDADMASCOTA' id='edadmascota' value="<?=$asis->asistencia_familia->EDADMASCOTA?>" size="2"></td></tr>
		<tr>
		<tr>
			<td><?=_('RAZA').' *'?></td>
			<td><input type='text' name='RAZA' id='raza' value="<?=$asis->asistencia_familia->RAZA?>" size="30"></td></tr>
		<tr>
		<tr>
			<td><?=_('COLOR').' *'?></td>
			<td><input type='text' name='COLOR' id='color' value="<?=$asis->asistencia_familia->COLOR?>" size="20"></td></tr>
		<tr>		
		<tr>
			<td><?=_('PEDIGREE')?></td>
			<td><input type='text' name='PEDIGREE' id='pedigree' value="<?=$asis->asistencia_familia->PEDIGREE?>" size="15"></td></tr>
		<tr>		
		</tr>
...
function validar_datos_generales(){
	...
	else if ($F('nombremascota')=='') alert("<?=_('INGRESE EL NOMBRE DE LA MASCOTA')?>");
