...
menu[1] = {
	...
	<?
	$parametro="?idasistencia=".$idasistencia;
	

	$sql = "select * from catalogo_etapa";
	$result = $con->query($sql);
	while($reg=$result->fetch_object())
	{
		$link=$reg->URL.$parametro;
		if ($idetapa_asis >= $reg->IDETAPA)
		{
			echo "[";
			
			if ($idetapa==$reg->IDETAPA) echo "'<br><FONT COLOR=RED>  $reg->DESCRIPCION ',";
			else 
	...
	["Media Jazz by Night xxxxxxxxxxxxxxxx", "http://www.mediajazzbynight.com/", "_new"],
