...
<body onLoad="document.fmrplanes.txtnombresocie.focus()">
		...
		<div id="tablewrapper">
			<div id="tableheader">
				...
				<div class="search">
					...
					<input type="text" name="txtnombresocie" size="72" value="<?=$_POST["txtnombresocie"]?>" style="text-transform:uppercase" class='classtexto' onFocus='coloronFocus(this);' onBlur="colorOffFocus(this)" />
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable">
				<thead>
					<tr>
						...
						<th class="nosort"><h3><? if(validar_permiso("CATSOCIEDADES_AGREGAR") ==1){?><input type="button" name="btnnuevo" id="btnnuevo" title="<?=_("AGREGAR NUEVA SOCIEDAD");?>" class='boton' onclick="reDirigir('add_catalogo.php')"  value="<?=_("NUEVO");?>"/><? } ?></h3></th>
					</tr>
				</thead>
				<tbody>
					<?							
						$Sql_costos="SELECT IDSOCIEDAD,NOMBRE,ACTIVO from $con->catalogo.catalogo_sociedad where NOMBRE like '%".$_POST["txtnombresocie"]."%'  ";

						$result=$con->query($Sql_costos);
						while($reg = $result->fetch_object()){
					?>
					<tr>
						<td style="text-align:center"><?=$reg->IDSOCIEDAD;?></td>
						<td><?=$reg->NOMBRE;?></td>
						<td><?=($reg->ACTIVO==1)?_("ACTIVO"):_("INACTIVO");?></td>
						<td align="center">						
							<? if(validar_permiso("CATSOCIEDADES_EDITAR") ==1){?><input type="button" name="btneditar" id="btneditar" value="<?=_("EDITAR");?>" class='boton' title="<?=_("EDITAR REGISTRO");?>" onclick="reDirigir('edit_catalogo.php?codigo=<?=$reg->IDSOCIEDAD ?>')"/><? } ?>
							<? if(validar_permiso("CATSOCIEDADES_EDITAR") ==1 and validar_permiso("CATSOCIEDADES_ELIMINAR") ==1){?>|<? } ?>
							<? if(validar_permiso("CATSOCIEDADES_ELIMINAR") ==1){?>
								<img src="../../../../imagenes/iconos/cancel.gif" border="0" style="cursor:pointer" title="<?=_("ELIMINAR REGISTRO");?>" onclick="confirmaRespuesta('<?=_("ESTAS SERGURO QUE DESEAS ELIMINAR EL REGISTRO: ".$reg->NOMBRE."?")?>','eliminacion.php?codigo=<?=$reg->IDSOCIEDAD?>')" border='0' width="16" height="14" title="<?=_("ELIMINAR") ;?>">
							<? } else{?>
