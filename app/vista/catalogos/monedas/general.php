...
<body onLoad="document.fmrplanes.txtnombremon.focus()">
		...
		<div id="tablewrapper">
			<div id="tableheader">
				...
				<div class="search">
					...
					<input type="text" name="txtnombremon" size="72" value="<?=$_POST["txtnombremon"]?>" style="text-transform:uppercase" class='classtexto' onFocus='coloronFocus(this);' onBlur="colorOffFocus(this)" />
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable">
				<thead>
					<tr>
						<th><h3><?=_("ID");?></h3></th>
						<th><h3><?=_("MONEDA");?></h3></th>
						<th><h3><?=_("SIMBOLO");?></h3></th>
						<th><h3><?=_("STATUS");?></h3></th>
						<th class="nosort"><h3><? if(validar_permiso("CATMONEDAS_AGREGAR") ==1){?><input type="button" name="btnnuevo" id="btnnuevo" title="<?=_("AGREGAR NUEVA MONEDA");?>" class='boton' onclick="reDirigir('add_catalogo.php')"  value="<?=_("NUEVO");?>" /><? } ?></h3></th>
					</tr>
				</thead>
				<tbody>
					<?					 						
						$Sql_moneda="SELECT IDMONEDA,DESCRIPCION,SIMBOLO,ACTIVO from $con->catalogo.catalogo_moneda where DESCRIPCION!='' and DESCRIPCION  like '%".$_POST["txtnombremon"]."%'  ";

						$result=$con->query($Sql_moneda);
						while($reg = $result->fetch_object()){							 
					?>
					<tr>
						<td style="text-align:center"><?=$reg->IDMONEDA;?></td>
						<td><?=$reg->DESCRIPCION;?></td>
						<td><?=$reg->SIMBOLO;?></td>
						<td><?=($reg->ACTIVO==1)?_("ACTIVO"):_("INACTIVO");?></td>
						<td align="center">						
							<? if(validar_permiso("CATMONEDAS_EDITAR") ==1){?><input type="button" name="btneditar" id="btneditar" value="<?=_("EDITAR");?>" class='boton' title="<?=_("EDITAR REGISTRO");?>" onclick="reDirigir('edit_catalogo.php?codigo=<?=$reg->IDMONEDA ?>')" /><? } ?>
							<? if(validar_permiso("CATMONEDAS_EDITAR") ==1 and validar_permiso("CATMONEDAS_ELIMINAR") ==1){?>|<? } ?>
							<? if(validar_permiso("CATMONEDAS_ELIMINAR") ==1){?>
								<img src="../../../../imagenes/iconos/cancel.gif" border="0" style="cursor:pointer" title="<?=_("ELIMINAR REGISTRO");?>" onclick="confirmaRespuesta('<?=_("ESTAS SERGURO QUE DESEAS ELIMINAR EL REGISTRO: ".$reg->DESCRIPCION."?")?>','eliminacion.php?codigo=<?=$reg->IDMONEDA?>')" border='0' width="16" height="14" title="<?=_("ELIMINAR") ;?>">
							<? } else{?>
