...
//consulta sociedad log
	$result=$con->query("SELECT * FROM catalogo_usuario_log WHERE IDUSUARIO='".$_GET["idusuario"]."' ORDER BY FECHAMOD DESC ");
?>
<HTML>
	<head>
	...
	<body onKeyDown="javascript:test(event);">
		<table border="0" cellpadding="1" cellspacing="1" class="catalogos" width="100%">
			<tr bgcolor="#336699">
				<td colspan="7"><font size="2px" color="#FFFFFF"><?=_("HISTORIAL DE CAMBIOS - CATALOGO DE USUARIOS");?></font></td></tr>
			<tr bgcolor="#000033">
				<td><font color="#FFFFFF"><?=_("FECHAMOD.");?></font></td>
				<td><font color="#FFFFFF"><?=_("USUARIOMOD.");?></font></td>
		...
		?>			
			<tr bgcolor="#FFFFDF" onMouseOver="this.style.backgroundColor='#e2ebef'; this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFDF'">
				...
				<td><b><?=$reg->IDUSUARIOMOD; ?></b></td>
				<td><b><?=$reg->NOMBRES; ?></b></td>
				<td><?=$reg->APELLIDOS; ?></td>
				<td><?=$reg->EMAIL; ?></td>
				<td><?=($reg->REINICIACONTRASENIA ==1)?"SI":"NO"; ?></td>
