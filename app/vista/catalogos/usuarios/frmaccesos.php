<?php
	...
	$rsusuaario=$con->query("SELECT IDMODULO FROM $con->temporal.seguridad_modulosxusuario where IDUSUARIO='$usuario' order by IDMODULO");
	while($regusu = $rsusuaario->fetch_object())
	 {
		$accesos[]=$regusu->IDMODULO; 
	 }

	$allvalor=$con->consultation("SELECT count(*) FROM $con->temporal.seguridad_acceso_cuenta where IDUSUARIO='$usuario' ");
	...
	$i=1;
	$rs_cuenta=$con->query("select IDCUENTA,IDPAIS from $con->temporal.seguridad_acceso_cuenta where IDUSUARIO='$usuario' order by IDCUENTA ");
	...
	$allcuentas= $con->consultation("SELECT TODOCUENTAS FROM $con->catalogo.catalogo_usuario WHERE IDUSUARIO='$usuario'  ");
	$cantidad=$con->consultation("select count(*)+4 from $con->catalogo.catalogo_cuenta order by NOMBRE ");	

	if(($cantidad[0][0]-3) <= $allvalor[0][0])	$valock=1;	else $valock=2;
...
<head>
	...
	<link rel="stylesheet" href="../../../../librerias/treeview/jquery.treeview.css" />
	<link rel="stylesheet" href="../../../../librerias/treeview/demo/screen.css" />
	<script type="text/javascript" src="../../../../librerias/jquery-ui-1.7.1/development-bundle/jquery-1.3.2.js"></script>	 
	<script src="../../../../librerias/treeview/lib/jquery.cookie.js" type="text/javascript"></script>
	<script src="../../../../librerias/treeview/jquery.treeview.js" type="text/javascript"></script>	
	<script type="text/javascript" src="../../../../librerias/treeview/demo/demo.js"></script>
...
<body>
 	<form name="frmAddPro" method="post" action="gaccesos.php" onSubmit="marcarTodos('chosen[]');" >
		...
		<div class="demo">
			<div id="tabs">
				...
				<div id="tabs-1">
					<table border="0" cellpadding="1" cellspacing="1" id="tabla_paises" style="border:1px solid #336600">
						...
						<tr><td bgcolor="#c6e1ff">							
								<table border="0" cellpadding="0" cellspacing="0" id="tabla_paises" style="font-size:12px">
									<tr><td>
											<div id="main">
												<ul id="red" class="treeview-red"> 
												 <?											
													$rsmenus=$con->query("SELECT substr(IDMODULO,6,30) as NOMBRECAT,IDMODULO,UPPER(substr(DESCRIPCION,10,20)) as datos, UPPER(DESCRIPCION) AS DESCRIPCION, TIPO FROM $con->temporal.seguridadmodulo where TIPO in('MENU_CAT') order by DESCRIPCION ");
													while($regmenu = $rsmenus->fetch_object()){
														...
														$rsusuaario=$con->query("SELECT IDMODULO FROM $con->temporal.seguridad_modulosxusuario where IDUSUARIO='$usuario' and IDMODULO LIKE '".$regmenu->NOMBRECAT."%' order by IDMODULO");
														while($regusu = $rsusuaario->fetch_object()){
															$indice=$indice+1;
															if($regusu->IDMODULO ==$regmenu->NOMBRECAT."_AGREGAR")	{ $marcagr="checked";	$valorsagr=$regusu->IDMODULO;	$disaag="disabled"; }
															if($regusu->IDMODULO ==$regmenu->NOMBRECAT."_EDITAR")	{ $marcaedit="checked"; $valorsedi=$regusu->IDMODULO; $disaed="disabled"; }
															if($regusu->IDMODULO ==$regmenu->NOMBRECAT."_ELIMINAR")	{	$marcaeli="checked";	$valorseli=$regusu->IDMODULO; $disael="disabled"; }																															
														}
														
														if($disaag!="" and $disaed!="" and $disael!="")		$disa="disabled";
							...
							<td bgcolor="#c6e1ff"><table border="0" cellpadding="0" cellspacing="0"  style="font-size:12px">
							<?		
								$i=0;	
								...
								<table border="0" cellpadding="0" cellspacing="0" id="tabla_paises" style="font-size:12px">
									<?
										...
										while($reg = $result->fetch_object()){
											...
											$ii=$ii+1;
											if(in_array($reg->IDMODULO,$accesos))	$marcar="checked";	else $disamod="disabled";
							...
							<td bgcolor="#c6e1ff"><table border="0" cellpadding="0" cellspacing="0" id="tabla_paises" style="font-size:12px">
							<?		
								$i=0;	
							...
							?>	
								<tr>
									<td><input type="checkbox" name="ckbpermisos[]" id="ckbpermisos" value="<?=$reg->IDMODULO; ?>" <? if($marcar and $marcar!="") echo $marcar; else echo $bloquearSeguridad;?> /></td>
									<td><?=strtoupper($reg->DESCRIPCION); ?></td>
								</tr>
							<?
									$marcar="";		
				...
				<div id="tabs-2" style="background:#c6e1ff">				
						<table width="420" border="0" cellpadding="1" cellspacing="0" style="border:1px solid #336600">
							<tr>
								<th WIDTH="170"  height="14" style="text-align:left; background-color:  #336600;color:#FFFFFF"><?=_("ACCIONES") ;?>:</th>
								<th height="14"></th>
							...
							<tr>
								<td height="150">
									<select name="avail" size="7" id="avail" MULTIPLE style='font-size:10px;width:300px; height:200px' onDblClick="javascript: moveVals(2, 'avail', 'chosen[]'); frmButtons(); marcarTodos('chosen[]'); return false;" onfocus='coloronFocus(this);' class='classtexto' onBlur='colorOffFocus(this);'>								
										<? 
 											$result=$con->query("select IDMODULO,DESCRIPCION from $con->temporal.seguridadmodulo WHERE TIPO='ACCION' and  IDMODULO not like 'CAT%' and IDMODULO not in (select IDMODULO from $con->temporal.seguridad_modulosxusuario  where IDUSUARIO='$usuario') ORDER BY DESCRIPCION ");
								...
								<td height="150">					  
								<? 
									$con->cmbselectdata("SELECT $con->temporal.seguridad_modulosxusuario.IDMODULO, $con->temporal.seguridadmodulo.DESCRIPCION from $con->temporal.seguridad_modulosxusuario inner join  $con->temporal.seguridadmodulo on $con->temporal.seguridadmodulo.IDMODULO= $con->temporal.seguridad_modulosxusuario.IDMODULO where $con->temporal.seguridad_modulosxusuario.IDUSUARIO='$usuario' and $con->temporal.seguridadmodulo.TIPO='ACCION' and seguridad_modulosxusuario.IDMODULO not like 'CAT%' order by $con->temporal.seguridadmodulo.DESCRIPCION","chosen[]",$selected,"MULTIPLE style='font-size:10px;width:300px; height:200px' onDblClick=\"javascript: moveVals(2, 'avail', 'chosen[]'); frmButtons(); return false;\" onfocus='coloronFocus(this);' class='classtexto' onBlur='colorOffFocus(this);'","2"); 
								?>
								</td>
							</tr>
						</table>			
				...
				<div id="tabs-3">
					<table height="66" border="0" cellpadding="1" cellspacing="1">
						<tr>
						<td width="285"><table width="137" border="0" cellpadding="1" cellspacing="1">
						  <tr>
							<td width="20"><label>
							  <input name="rdseleccion" type="radio" id="radio" value="1" <?=($allcuentas[0][0]==1)?"checked":"";?> onclick="mostrar_div('resultado','none')">
							</label></td>
							<td style="font-size:14px"><?=_("TODOS") ;?></td>
						  </tr>
						  <tr>
							<td><input type="radio" name="rdseleccion" id="radio2" value="2"  <?=($allcuentas[0][0]==0)?"checked":"";?> onclick="mostrar_div('resultado','block')"></td>
							<td style="font-size:14px"><?=_("POR CUENTAS") ;?></td>
						  </tr>
						</table></td>
						</tr>
					</table>
											
					<div id="resultado" Style="display:<?=($allcuentas[0][0]==1)?"none":"block";?>;overflow:auto;padding-top:1px; padding-Left:1px; padding-bottom:15px;height:198px; width:300px;">			
						...
						?>								
								  <tr bgcolor="<?=$fondo;?>">
									<td colspan="2"><input type="checkbox" name="chkcuentas[]"  id="chkcuentas<?=$c?>" value="<?=$reg->IDCUENTA; ?>" <?=$valor;?> onclick="mostrar_plan('<?=$reg->IDCUENTA?>','<?=$c?>','<?=$reg->NOMBRE?>')"  ><strong><?=$reg->NOMBRE; ?></strong></td>
								  </tr>
						<?
								$valor="";																	
								$c=$c+1;
					...
					<div id="div-plan" style="position:absolute;overflow:auto;margin:1px;padding:1px;float:left;position:absolute;top:115px;left:425px;height:198px;width:400px"></div>
						<div id="verseleccion" Style="display:<?=($allcuentas[0][0]==1)?"none":"block";?>">
	...
	<script type="text/javascript">
		function mostrar_plan(idcodigo,conta,cuenta){
		 
		/* 	if(document.getElementById('chkcuentas'+conta).checked ==true){
				 
			} else{
			
				document.getElementById('div-plan').style.display ='none';
			} */
		}
		
		function ocultar_plan(idcodigo,conta){
		 
			if(document.getElementById('chkcuentas'+conta).checked ==false){
