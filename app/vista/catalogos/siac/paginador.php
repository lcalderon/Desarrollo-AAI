 	...
 	$Sql="SELECT
		...
		if($_REQUEST["cmbbusqueda"]!="null" and $_REQUEST["cmbbusqueda"]!="")
		 {		
			...
			if($_REQUEST["cmbbusqueda"] ==7) $SqlFinal=$Sql." group by catalogo_afiliado.IDAFILIADO,catalogo_afiliado_beneficiario.IDBENEFICIARIO order by catalogo_afiliado_beneficiario.APPATERNO,catalogo_afiliado_beneficiario.APMATERNO ";
			else $SqlFinal=$Sql." group by catalogo_afiliado.IDAFILIADO order by catalogo_afiliado_persona.APPATERNO,catalogo_afiliado_persona.APMATERNO";
...
?>
	<table cellpadding="1" cellspacing="1" border="0" id="table" class="tinytable" style="width:100%">
		<tr bgcolor='#597D98'>
			...
			<th class="nosort"><h3></h3></th>
			<? if($_REQUEST['cuenta'] =="LIF"){ ?><th class="nosort"><h3></h3></th><? } ?>
		</tr> 
<?
...
?>
		 <tr bgcolor=<?=$fondo;?> class='<?=$clase;?>' title="<?=$reg->CVEAFILIADO." - ".$reg->nombres;?>" >
			...
			<? if($_REQUEST['cuenta'] =="LIF" || $reg->IDCUENTA =="LIF"){ ?><td><a href="#" onClick="window.open('../../lifecare/detalles.php?idafiliado=<?=$reg->IDAFILIADO?>','lifecare','height=450, width=1500,left=100,top=0,resizable=no,scrollbars=yes,toolbar=no,status=yes');">LIFECARE</a></td><? } ?>
...
//desplazamiento 
	...
	echo "<a onclick=\"paginar_total('1',1)\" style='text-decoration:underline;cursor:pointer' title='Primero'>Primera</a> "; 
	if($PagAct>1) echo "<a onclick=\"paginar_total('$PagAnt',1)\" style='text-decoration:underline;cursor:pointer' title='Anterior'>Anterior</a> ";
	echo "<strong>Pagina ".$PagAct."/".$PagUlt."</strong>";
	if($PagAct<$PagUlt)  echo " <a onclick=\"paginar_total('$PagSig',1)\" style='text-decoration:underline;cursor:pointer' title='Siguiente'>Siguiente</a> ";
	if($NroRegistros > $RegistrosAMostrar) echo " <a onclick=\"paginar_total('$PagUlt',1)\" style='text-decoration:underline;cursor:pointer' title='Ultimo'>Ultimo</a>";
