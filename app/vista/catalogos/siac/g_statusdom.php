	...
	if($con->Errno)
	 {
		...
		$respuesta=$con->update("$con->catalogo.catalogo_afiliado_persona_domicilio_ubigeo",$row,"where ID='".$_POST['id']."' ");	
		
		$row2["ID"]=$_POST["id"];
		$row2["IDAFILIADO"]=$_POST["idcodigo"];
		$row2["ACTIVO"]=$_POST["valor"];		
		$row2["IDUSUARIOMOD"]=$_SESSION["user"];		
		if($respuesta)	$con->insert_reg("$con->catalogo.catalogo_afiliado_persona_domicilio_ubigeo_log",$row2);
