...
<body onLoad="document.fmrplanes.txtnombrecosto.focus()">
		...
		<div id="tablewrapper">
			<div id="tableheader">
				...
				<div class="search">
					...
					<input type="text" name="txtnombrecosto" size="72" value="<?=$_POST["txtnombrecosto"]?>" class='classtexto' style="text-transform:uppercase" onFocus='coloronFocus(this);' onBlur="colorOffFocus(this)" />
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable">
				<thead>
					<tr>
						...
						<th><h3><?=_("NEGOCIADO");?></h3></th>
						<th><h3><?=_("STATUS");?></h3></th>
						<th class="nosort"><h3><? if(validar_permiso("CATCOSTOS_AGREGAR") ==1){?><input type="button" name="btnnuevo" id="btnnuevo" title="<?=_("AGREGAR NUEVO COSTO");?>" class='boton' onclick="reDirigir('add_catalogo.php')"  value="<?=_("NUEVO");?>" /><? } ?></h3></th>
					</tr>
				</thead>
				<tbody>
					<?							 						
						$Sql_costo="select catalogo_costo.IDCOSTO,catalogo_costo.DESCRIPCION,if(catalogo_costo.COSTONEGOCIADO=1,'OK','') AS COSTONEGOCIADO ,catalogo_costo.ACTIVO from $con->catalogo.catalogo_costo where catalogo_costo.DESCRIPCION like '%".$_POST["txtnombrecosto"]."%'  ";

						$result=$con->query($Sql_costo);
						while($reg = $result->fetch_object()){							 
					?>
					<tr>
						<td style="text-align:center"><?=$reg->IDCOSTO;?></td>
						<td><?=$reg->DESCRIPCION;?></td>
						<td><?=$reg->COSTONEGOCIADO;?></td>
						<td><?=($reg->ACTIVO==1)?_("ACTIVO"):_("INACTIVO");?></td>
						<td align="center">						
							<? if(validar_permiso("CATCOSTOS_EDITAR") ==1){?><input type="button" name="btneditar" id="btneditar" value="<?=_("EDITAR");?>" class='boton' title="<?=_("EDITAR REGISTRO");?>" onclick="reDirigir('edit_catalogo.php?codigo=<?=$reg->IDCOSTO ?>')"  /><? } ?>
							<? if(validar_permiso("CATCOSTOS_EDITAR") ==1 and validar_permiso("CATCOSTOS_ELIMINAR") ==1){?>|<? } ?>
							<? if(validar_permiso("CATCOSTOS_ELIMINAR") ==1){?>
								<img src="../../../../imagenes/iconos/cancel.gif" border="0" style="cursor:pointer" title="<?=_("ELIMINAR REGISTRO");?>" onclick="confirmaRespuesta('<?=_("ESTAS SERGURO QUE DESEAS ELIMINAR EL REGISTRO: ".$reg->DESCRIPCION."?")?>','eliminacion.php?codigo=<?=$reg->IDCOSTO?>')" border='0' width="16" height="14" title="<?=_("ELIMINAR") ;?>">
							<? } else{?>
