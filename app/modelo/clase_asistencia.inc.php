...
class asistencia extends DB_mysqli{
	...
	*/
	...
	function listar_asistencias($idusuario='',$arrstatusexpediente='PRO',$arrstatusasistencia,$idetapas,$idcuentas,$modo='',$statusautorizaciondesvio='0',$annio,$mes,$dia,$hora,$annio2,$mes2,$dia2,$hora2,$idasistencia,$statusnegociacionposterior,$campo,$dato,$opc)
	{
		...
		switch ($campo){
			...
			case 'a.IDASISTENCIA':
				{
					if ($dato!=''){
						$condicion1.=" $campo='$dato'  AND e.ARRSTATUSEXPEDIENTE='$arrstatusexpediente' ";
						$condicion2.=" a.IDASISTENCIA <>'S/D'";
					}
					break;
				}
			default:
				{
					...
					if ($idusuario!='') {
						$condicion1.= " a.IDUSUARIORESPONSABLE in ( $idusuario)";
					    $condicion2.= " e.IDUSUARIOMOD in ($idusuario) ";
					}
                    if ($arrstatusasistencia!='') $condicion1.= " AND a.ARRSTATUSASISTENCIA = '$arrstatusasistencia'";
					    
					if ($annio2=='' &&  $mes2=='' && $dia2=='' )
					{
						if ($annio!=''){
							$condicion1.= " AND YEAR(e.FECHAREGISTRO)=$annio ";
							$condicion2.= " AND YEAR(e.FECHAREGISTRO)=$annio ";
						}
						if ($mes!='') {
							$condicion1.= " AND MONTH(e.FECHAREGISTRO)=$mes ";
							$condicion2.= " AND MONTH(e.FECHAREGISTRO)=$mes ";
						}
						if ($dia!='') {
							$condicion1.= " AND DAY(e.FECHAREGISTRO)=$dia";
							$condicion2.= " AND DAY(e.FECHAREGISTRO)=$dia";
						}
						if ($hora!='') {
							$condicion1.= " AND HOUR(e.FECHAREGISTRO)=$hora ";
							$condicion2.= " AND HOUR(e.FECHAREGISTRO)=$hora ";
						}
					}
					else if ($hora2=='' && $hora=='')
					{
						$condicion1.=" AND (e.FECHAREGISTRO BETWEEN '$annio-$mes-$dia $hora:00:00' AND '$annio2-$mes2-$dia2 23:59:59' )";
						$condicion2.=" AND (e.FECHAREGISTRO BETWEEN '$annio-$mes-$dia $hora:00:00' AND '$annio2-$mes2-$dia2 23:59:59' )";
					}
					else
					{
						$condicion1.=" AND (e.FECHAREGISTRO BETWEEN '$annio-$mes-$dia $hora:00:00' AND '$annio2-$mes2-$dia2 $hora2:59:59' )";
						$condicion2.=" AND (e.FECHAREGISTRO BETWEEN '$annio-$mes-$dia $hora:00:00' AND '$annio2-$mes2-$dia2 $hora2:59:59' )";
					}

                           
					if ($statusnegociacionposterior!='') $condicion1.=" AND a.STATUSNEGOCIACIONPOSTERIOR= '$statusnegociacionposterior'";
					
					if ($statusautorizaciondesvio==0) $condicion1.= " AND a.STATUSAUTORIZACIONDESVIO=0 AND a.IDUSUARIOAUTORIZACIONDESVIO=''";
					elseif ($statusautorizaciondesvio==1)  $condicion1.= " AND a.STATUSAUTORIZACIONDESVIO=1";

					if ($modo=='DESVIO') $condicion1.= " AND a.DESVIACION=1";
					else
					$condicion1.= " AND a.IDETAPA in ( $idetapas)";
...
e.IDCUENTA, 
e.IDPROGRAMA, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',a.IDASISTENCIA) IDASISTENCIA, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',a.IDPROGRAMASERVICIO) IDPROGRAMASERVICIO, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',a.IDSERVICIO) IDSERVICIO, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',a.ARRSTATUSASISTENCIA) ARRSTATUSASISTENCIA, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',a.ARRPRIORIDADATENCION) ARRPRIORIDADATENCION, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',a.ARRCONDICIONSERVICIO) ARRCONDICIONSERVICIO, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',a.IDETAPA) IDETAPA, 
IF (a.IDEXPEDIENTE IS NULL ,e.IDUSUARIOMOD,a.IDUSUARIORESPONSABLE) IDUSUARIORESPONSABLE, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',cs.DESCRIPCION ) AA_SERVICIO, 
IF (a.IDEXPEDIENTE IS NULL ,'S/D',cps.ETIQUETA) NOM_SERVICIO, 
IF (a.IDEXPEDIENTE IS NULL ,e.FECHAREGISTRO, MIN(au.FECHAHORA)) FECHAHORA,
IF (a.IDEXPEDIENTE IS NULL ,'S/D',cprov.NOMBRECOMERCIAL) NOM_PROVEEDOR,
IF (a.IDEXPEDIENTE IS NULL ,'S/D',ce.DESCRIPCION)  NOM_ETAPA,
a.DESVIACION, 
...
LEFT JOIN $this->temporal.asistencia a ON a.IDEXPEDIENTE = e.IDEXPEDIENTE
...
e.ARRSTATUSEXPEDIENTE = '$arrstatusexpediente'
AND e.IDCUENTA in ($idcuentas) 

AND (
(
$condicion1
AND ( aap.IDPROVEEDOR =(	SELECT 
							aap1.idproveedor
							FROM 
